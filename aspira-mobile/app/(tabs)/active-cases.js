...
import {
  Clock3, CheckCircle2, XCircle, LoaderCircle,
  FileQuestion, ChevronRight, AlertCircle, MapPin, CalendarDays,
} from "lucide-react-native";
import AsyncStorage from "@react-native-async-storage/async-storage";
import { api } from "../../services/api";

const STATUS_CONFIG = {
  pending:       { label: "Menunggu",      color: "#b07d00", bg: "#fff7d6", icon: Clock3       },
  diperiksa:     { label: "Diperiksa",     color: "#004b8d", bg: "#e8f5ff", icon: LoaderCircle },
  diproses:      { label: "Diproses",      color: "#004b8d", bg: "#e8f5ff", icon: LoaderCircle },
  diverifikasi:  { label: "Diverifikasi",  color: "#6d28d9", bg: "#ede9fe", icon: LoaderCircle },
  tindak_lanjut: { label: "Tindak Lanjut", color: "#0369a1", bg: "#e0f2fe", icon: LoaderCircle },
  selesai:       { label: "Selesai",       color: "#0a7c5c", bg: "#e6f9f4", icon: CheckCircle2 },
  ditolak:       { label: "Ditolak",       color: "#c0392b", bg: "#fde8e8", icon: XCircle      },
  rejected:      { label: "Ditolak",       color: "#c0392b", bg: "#fde8e8", icon: XCircle      },
};

const PRIORITY_CONFIG = {
  urgent: { label: "Prioritas Mendesak", color: "#ef4444" },
  high:   { label: "Prioritas Tinggi",   color: "#ef4444" },
  medium: { label: "Prioritas Sedang",   color: "#f59e0b" },
  low:    { label: "Prioritas Rendah",   color: "#3b82f6" },
};
...
export default function ActiveCasesScreen() {
  ...
  const renderItem = ({ item }) => {
    const status       = STATUS_CONFIG[item.status]                    || { label: item.status, color: "#6b7280", bg: "#f9fafb", icon: null };
    ...
    <View style={styles.container}>
      ...
      {!error && reports.length === 0 ? (
        ...
        >
          ...
          <TouchableOpacity style={styles.createButton} onPress={() => router.push("/(tabs)/create")}>
