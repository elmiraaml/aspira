...
export default function EditReportPage() {
  ...
  const [existingImageUrl, setExistingImageUrl] = useState(""); // ← simpan URL foto lama
    ...
    const loadData = async () => {
      try {
        ...
        if (fotoUrl) {
          ...
          setExistingImageUrl(fotoUrl); // ← simpan URL asli
        }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      let imageUrl = existingImageUrl; // ← default pakai foto lama
      ...
      if (formData.bukti_foto) {
        ...
        imageUrl = uploadData.image_url; // ← ganti dengan foto baru
      }
      ...
      const payload = {
        ...
        image: imageUrl || null, // ← selalu kirim image
      };
      ...
      <div className="flex flex-col flex-1 min-w-0">
        <main className="flex-1 px-8 py-7 flex justify-center">
          <div className="w-full max-w-2xl flex flex-col gap-6">
            ...
            <Link
              href={`/user/report/${id}`}
              className="inline-flex items-center gap-2 text-blue-600 font-semibold text-sm hover:gap-3 transition-all w-fit"
            >
              <ArrowLeft size={18} />
              Kembali ke Detail
            </Link>
