...
export default function UserReportDetailPage() {
  ...
  const fetchDetail = async () => {
    try {
      ...
      if (res.message === "Server error") { console.error("Gagal ambil detail laporan:", res); return; }
  ...
  const fetchComments = async () => {
    try {
      ...
      if (res?.message?.includes("Cannot GET")) { setComments([]); return; }
      if (res.message === "Server error") { console.error("Fetch Comment Error:", res); return; }
  ...
  // Geocode nama lokasi → koordinat untuk peta
  useEffect(() => {
    if (!report?.location) return;

    const loc = report.location;

    // Cek apakah sudah format koordinat "lat, lng"
    ...
    // Geocode nama tempat via Nominatim
    const geocode = async () => {
      try {
  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      if (res?.message?.includes("Cannot POST")) { alert("Fitur komentar belum tersedia di backend."); return; }
      if (res.message && res.message !== "Komentar berhasil ditambahkan") { alert(res.message || "Gagal menambahkan komentar"); return; }
      setNewComment("");
      fetchComments();
    } catch (error) {
  ...
  if (!report) {
    return (
      <div className="text-center p-16 bg-white rounded-3xl border border-blue-50 shadow-sm flex flex-col items-center">
        ...
        <Link href="/user" className="text-blue-600 font-semibold hover:underline">Kembali ke Dashboard</Link>
      </div>
    );
  }
                ...
                <div className="absolute right-0 top-12 z-50 w-52 bg-white rounded-2xl border border-gray-100 shadow-xl overflow-hidden">
                  {report.status === "pending" && (
                    ...
                    >
                      ...
                      <span className="text-sm font-semibold text-blue-600">Edit Laporan</span>
                    </Link>
                  )}
                  <button
                  ...
                  >
                    ...
                    <span className="text-sm font-semibold text-red-500">Hapus Laporan</span>
                  </button>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col gap-3 mb-6">
              {comments.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className={`p-4 rounded-2xl border ${isAdmin ? "bg-blue-50/50 border-l-4 border-l-blue-600 border-transparent" : "bg-gray-50/50 border-gray-100"}`}
                  >
